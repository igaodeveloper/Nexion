...
import { Clock, Eye, ChevronLeft, ChevronRight, SkipBack, RotateCcw } from 'lucide-react';
...
export function TimeMachine({ documentId, currentPage, onRestore }: TimeMachineProps) {
  const [open, setOpen] = useState(false);
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(null);
  const [sliderValue, setSliderValue] = useState(100); // 100 means current version
  const [viewMode, setViewMode] = useState<'slider' | 'grid' | 'compare'>('slider');
  ...
  const selectedVersion = selectedVersionId && versions 
  ...
  const handleNextVersion = () => {
    ...
    if (currentIndex < versions.length - 1) {
      ...
      setSliderValue(Math.round(((currentIndex + 1) / (versions.length - 1)) * 100));
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-4xl h-[80vh] flex flex-col">
        ...
        <Tabs defaultValue="slider" className="flex-1 flex flex-col" onValueChange={(value) => setViewMode(value as any)}>
            ...
            <div className="mt-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="icon" onClick={() => setSelectedVersionId(null)}>
                    <SkipBack className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="icon" onClick={handlePreviousVersion}>
                    <ChevronLeft className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="icon" onClick={handleNextVersion}>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div 
                className={cn(
                  ...
                  selectedVersionId === null ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"
                )}
              ...
              >
                ...
                <p className="text-sm font-medium truncate">{currentPage.title}</p>
              ...
              {versions?.map((version) => (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    Versão {version.versionNumber} - {formatDate(version.createdAt)}
                  </p>
                </div>
              ))}
            </div>
          ...
          {/* Compare View */}
          <TabsContent value="compare" className="flex-1 grid grid-cols-2 gap-4">
