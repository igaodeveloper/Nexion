...
import { 
  MessageCircle, MoreHorizontal, X, CheckCircle, Reply, Smile, Edit, 
  Flag, Copy, Send, Plus, Heart, ThumbsUp, Loader2, AlertTriangle, 
...
export function CommentList({ documentId }: CommentListProps) {
  ...
  const { data: comments, isLoading: commentsLoading, refetch } = useQuery<Comment[]>({
  ...
  const addCommentMutation = useMutation({
    mutationFn: async (newComment: { text: string; documentId: number; parentId?: number }) => {
  ...
  const toggleReactionMutation = useMutation({
    mutationFn: async ({ commentId, emoji }: { commentId: number; emoji: string }) => {
      const response = await apiRequest('POST', '/api/comments/reactions', { commentId, emoji });
      return await response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/documents', documentId, 'reactions'] });
    },
  });
  ...
  const resolveCommentMutation = useMutation({
    mutationFn: async (commentId: number) => {
      const response = await apiRequest('PATCH', `/api/comments/${commentId}/resolve`);
      return await response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/documents', documentId, 'comments'] });
  ...
  const getGroupedReactions = (commentId: number) => {
    ...
    return commentReactions.reduce((acc, reaction) => {
  ...
  const renderComment = (comment: Comment, isReply = false) => {
    ...
    return (
      <div key={comment.id} className={cn(
        "mb-4 last:mb-0",
        isReply ? "ml-8 mt-2" : "border-t border-border pt-4 first:border-t-0 first:pt-0"
      )}>
        <div className="flex gap-3">
          <Avatar className="h-8 w-8 border shadow-sm">
            <AvatarImage src={user?.avatar || ''} />
            <AvatarFallback className="bg-primary/10 text-primary">{getUserInitials(user)}</AvatarFallback>
          </Avatar>
            ...
            <div className="flex items-center justify-between">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  {!comment.resolved && (
                    <DropdownMenuItem onClick={() => resolveCommentMutation.mutate(comment.id)} className="cursor-pointer">
                      <CheckCircle className="h-4 w-4 mr-2" />
                      <span>Resolver</span>
                    </DropdownMenuItem>
                  )}
                  <DropdownMenuItem onClick={() => setReplyToId(comment.id)} className="cursor-pointer">
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              ...
              <div className="relative">
                ...
                {showEmojiPicker === comment.id && (
                  <EmojiPicker 
                    onSelect={(emoji: string) => handleReactionClick(comment.id, emoji)}
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          {comments && comments.length > 0 && (
            <Badge variant="outline" className="text-xs rounded-full h-5 px-1.5 bg-primary/5 hover:bg-primary/5">
              {comments.length}
            </Badge>
          )}
        </div>
        
        <Button variant="ghost" size="sm" className="h-7 gap-1" onClick={() => refetch()}>
